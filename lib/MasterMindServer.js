...
class Player {
    ...
    on(msgId, cbFcn) {
        ...
        this.socket.once(msgId, (...args) => {
            log("Got message " + msgId + ' for ' + this.uuid);
            cbFcn(...args);
        });
    }
...
class Game {
    ...
    iscorrect(setup_, guess_, score) {
        ...
        let ok = (nWhite == nWhiteCorrect) && (nBlack == nBlackCorrect);
        let won = (nBlackCorrect == 4);
        return [ok, won];
    }
    ...
    async play() {
        ...
        while (1) {
            ...
            let guess = await this.player1.getReply('guess', {score, setup: [], won, gameover: false});
            log('player1 guessed with ' + guess);

            while (1) {
                log('asking player2 to score');
                score = await this.player2.getReply('score', {guess, won, gameover: false});
                log('player2 scored with ' + score);

                let ok = false;
                [ok, won] = this.iscorrect(setup, guess, score);
        ...
        await this.player1.sendCmd('guess', {score, setup, won, gameover: true});
        await this.player2.sendCmd('score', {guess: [], won, gameover: true});
    }
}
